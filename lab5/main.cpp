...
void generarGraficaAFD(AFD &afd,ostream &os){
...
void generarGraficaAFN(AFN &afd,ostream &os){
    os<<"digraph finite_state_machine {"<<endl;
	os<<"fontname=\"Helvetica,Arial,sans-serif\""<<endl;
	os<<"node [fontname=\"Helvetica,Arial,sans-serif\"]"<<endl;
	os<<"edge [fontname=\"Helvetica,Arial,sans-serif\"]"<<endl;
...
{ 
    AFN afn;
    AFD afd;
    string entrada,salida,entradaGrafica;
    ...
    cout<<"Nombre del archivo grafica AFD";getline(cin,entradaGrafica);
    ...
    generarGraficaAFD(afd,archivo);
    generarGraficaAFN(afn,archivo);

    string comand = "dot -Tpng "+salida+" -o "+salida.substr(0,salida.size()-4) +".png";
    system(comand.c_str());
